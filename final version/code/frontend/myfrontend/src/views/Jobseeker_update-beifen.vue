<template>
  <div class="container">
    ...
    <!-- 职业偏好 -->
    <h3>职业偏好</h3>
    <form @submit.prevent="updateDesiredIndustry">
      <div>
        <label>喜爱行业：</label>
    ...
    <form @submit.prevent="updateDesiredJobTitle">
      <div>
        <label>喜爱岗位：</label>
    ...
    <form @submit.prevent="updateDesiredCity">
      <div>
        <label>喜爱城市：</label>
  ...
  try {
    if (form.value.language_id && form.value.language_proficiency) {
      const languageData = {
        ...
        language: {
          id: form.value.language_id,
          name: availableLanguages.value.find(lang => lang.id === form.value.language_id)?.name || ''
        },
      ...
      const languageResponse = await updateJobSeekerLanguage(parseInt(id.value), languageData);
...
const updateEducation = async () => {
  try {
    if (form.value.education_experience.school_id && form.value.education_experience.degree) {
      ...
      const educationResponse = await updateEducationExperience(parseInt(id.value), educationData);
...
const updateDesiredInd = async () => {
  try {
    if (form.value.desired_industry_id) {
      ...
      const industryResponse = await updateDesiredIndustry(parseInt(id.value), industryData);
...
const updateDesiredJobT = async () => {
  try {
    if (form.value.desired_job_title_id) {
      ...
      const jobTitleResponse = await updateDesiredJobTitle(parseInt(id.value), jobTitleData);
...
const updateDesiredC = async () => {
  try {
    if (form.value.desired_city_id) {
      ...
      const cityResponse = await updateDesiredCity(parseInt(id.value), cityData);
